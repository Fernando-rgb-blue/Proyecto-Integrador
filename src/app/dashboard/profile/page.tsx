  ...
  return (
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-5">
      ...
      <div className="w-full text-left mb-0">
        <h1 className="text-2xl font-bold">Bienvenido Docente : {session?.user?.fullname} </h1>
        <p className="text-gray-600">Cambia tu contraseña o registra tu horario con el tiempo que tengas libre.</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-md text-center mb-4">
        <p className="font-medium text-lg">Nombre: {session?.user?.fullname || "Nombre de usuario"}</p>
        <p className="text-gray-600">Correo: {session?.user?.email || "Correo de usuario"}</p>
      </div>
